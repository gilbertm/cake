...
class Merlin {
	...
	function __construct( $config = array(), $strings = array() ) {
		...
		add_action( 'eva_custom_fields_import', array( $this, 'import_custom_fields_and_taxonomies' ) );
	}
	...
	public function admin_page() {
		...
		$texts = array(
			'something_went_wrong' => esc_html__( 'Something went wrong. Please refresh the page and try again!', 'eva' ),
		);
	...
	public function svg( $args = array() ) {
		...
		if ( empty( $args ) ) {
			return __( 'Please define default parameters in the form of an array.', 'eva' );
		}

		// Define an icon.
		if ( false === array_key_exists( 'icon', $args ) ) {
			return __( 'Please define an SVG icon filename.', 'eva' );
		}
	...
	public function steps() {
		...
		if ( ! empty( $this->import_files ) ) {
			$this->steps['content'] = array(
				'name' => esc_html__( 'Content', 'eva' ),
				'view' => array( $this, 'content' ),
			);
		}

		$this->steps['ready'] = array(
			'name' => esc_html__( 'Ready', 'eva' ),
	...
	protected function welcome() {
		...
		<?php
		$this->logger->debug( __( 'The welcome step has been displayed', 'eva' ) );
	}
	...
	protected function license() {
		...
		<?php
		$this->logger->debug( __( 'The license activation step has been displayed', 'eva' ) );
	}
	...
	protected function eva_info() {
		...
		<div class="svg_holder">
			...
			<h1><?php esc_html_e( 'Server Requirements', 'eva' ); ?></h1>

			<p id="child-theme-text"><?php esc_html_e( 'Values are just a recommendation and not a strict requirement. But in the end, if the demo installation fails then please make sure that all values are met.', 'eva' ); ?></p>
			...
			<?php if ( ! empty( $checks ) ) : ?>
				<div class="woodstock--step-info merlin__drawer--open">
					...
					<a class="merlin__button merlin__button--knockout merlin__button--no-chevron merlin__button--external"
					   ...
					   target="_blank"><?php esc_html_e( 'Learn how to increase values', 'eva' ); ?></a>
				</div>
			<?php endif; ?>
		...
		<?php
		$this->logger->debug( __( 'The server info installation step has been displayed', 'eva' ) );
	}
	...
	protected function child() {
		...
		<?php
		$this->logger->debug( __( 'The child theme installation step has been displayed', 'eva' ) );
	}
	...
	protected function plugins() {
						...
						<?php foreach ( $required_plugins as $slug => $plugin ) : ?>
							<li data-slug="<?php echo esc_attr( $slug ); ?>">
								...
								<label for="default_plugins_<?php echo esc_attr( $slug ); ?>">
									...
									<span class="badge">
									<span class="hint--top"
										  aria-label="<?php esc_html_e( 'Required', 'eva' ); ?>">
										<?php esc_html_e( 'req', 'eva' ); ?>
									</span>
								</span>
								</label>
							</li>
		...
		<?php
		$this->logger->debug( __( 'The plugin installation step has been displayed', 'eva' ) );
	}
	...
	protected function content() {
			...
			<?php if ( 1 < count( $this->import_files ) ) : ?>
				...
				<div class="merlin__select-control-wrapper">
					...
					<div class="merlin__select-control-help">
						<span class="hint--top" aria-label="<?php echo esc_attr__( 'Select Demo', 'eva' ); ?>">
		...
		<?php
		$this->logger->debug( __( 'The content import step has been displayed', 'eva' ) );
	}
	...
	protected function ready() {
		...
		<?php
		$this->logger->debug( __( 'The final step has been displayed', 'eva' ) );
	}
	...
	public function generate_child() {
		...
		if ( ! file_exists( $path ) ) {
			...
			$this->logger->debug( __( 'The existing child theme was activated', 'eva' ) );
		...
		$this->logger->debug( __( 'The newly generated child theme was activated', 'eva' ) );
	...
	public function _ajax_activate_license() {
		...
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) ) {
			wp_send_json(
				array(
					'success' => false,
					'message' => esc_html__( 'Yikes! The theme activation failed. Please try again or contact support.', 'eva' ),
				)
			);
		}
		...
		if ( empty( $_POST['license_key'] ) ) {
			wp_send_json(
				array(
					'success' => false,
					'message' => esc_html__( 'Please add your license key before attempting to activate one.', 'eva' ),
				)
			);
		}
		...
		$this->logger->debug( __( 'The license activation was performed with the following results', 'eva' ), $result );
	...
	protected function edd_activate_license( $license ) {
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			...
			if ( is_wp_error( $response ) ) {
				...
				$message = esc_html__( 'An error occurred, please try again.', 'eva' );
			}
		} else {
				...
				switch ( $license_data->error ) {
					...
					case 'expired':
						...
						/* translators: Expiration date */
							esc_html__( 'Your license key expired on %s.', 'eva' ),
							date_i18n( get_option( 'date_format' ), strtotime( $license_data->expires, current_time( 'timestamp' ) ) )
						);
						break;

					case 'revoked':
						$message = esc_html__( 'Your license key has been disabled.', 'eva' );
						break;

					case 'missing':
						$message = esc_html__( 'This appears to be an invalid license key. Please try again or contact support.', 'eva' );
						break;

					case 'invalid':
					case 'site_inactive':
						$message = esc_html__( 'Your license is not active for this URL.', 'eva' );
						break;

					case 'item_name_mismatch':
						/* translators: EDD Item Name */
						$message = sprintf( esc_html__( 'This appears to be an invalid license key for %s.', 'eva' ), $this->edd_item_name );
						break;

					case 'no_activations_left':
						$message = esc_html__( 'Your license key has reached its activation limit.', 'eva' );
						break;
	...
	public function generate_child_functions_php( $slug ) {
		...
		$this->logger->debug( __( 'The child theme functions.php content was generated', 'eva' ) );
	...
	public function generate_child_style_css( $slug, $parent, $author, $version ) {
		...
		$this->logger->debug( __( 'The child theme style.css content was generated', 'eva' ) );
	...
	public function generate_child_screenshot( $path ) {
		...
		if ( ! empty( $screenshot ) && file_exists( $screenshot ) ) {
			...
			$this->logger->debug( __( 'The child theme screenshot was copied to the child theme, with the following result', 'eva' ), array( 'copied' => $copied ) );
		} else {
			$this->logger->debug( __( 'The child theme screenshot was not generated, because of these results', 'eva' ), array( 'screenshot' => $screenshot ) );
		}
	}
	...
	function _ajax_plugins() {
		...
		foreach ( $plugins['activate'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Activating', 'eva' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['update'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Updating', 'eva' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['install'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Installing', 'eva' ),
				);
				break;
			}
		}
		...
		if ( $json ) {
			$this->logger->debug(
				__( 'A plugin with the following data will be processed', 'eva' ),
				array(
			...
			$json['message'] = esc_html__( 'Installing', 'eva' );
			wp_send_json( $json );
		} else {
			$this->logger->debug(
				__( 'A plugin with the following data was processed', 'eva' ),
				array(
					'plugin_slug' => $_POST['slug'],
				)
			);
			...
			wp_send_json(
				array(
					'done'    => 1,
					'message' => esc_html__( 'Success', 'eva' ),
				)
			);
		}
	...
	function _ajax_content() {
		...
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) || empty( $_POST['content'] ) && isset( $content[ $_POST['content'] ] ) ) {
			$this->logger->error( __( 'The content importer AJAX call failed to start, because of incorrect data', 'eva' ) );

			wp_send_json_error(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Invalid content!', 'eva' ),
				)
			);
		}
		...
		if ( isset( $_POST['proceed'] ) ) {
			if ( is_callable( $this_content['install_callback'] ) ) {
				$this->logger->info(
					__( 'The content import AJAX call will be executed with this import data', 'eva' ),
					array(
		...
		if ( $json ) {
			...
			$this->logger->error(
				__( 'The content import AJAX call failed with this passed data', 'eva' ),
				array(
	...
	public function _ajax_get_total_content_import_items() {
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) && empty( $_POST['selected_index'] ) ) {
			$this->logger->error( __( 'The content importer AJAX call for retrieving total content import items failed to start, because of incorrect data.', 'eva' ) );

			wp_send_json_error(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Invalid data!', 'eva' ),
				)
			);
		}
	...
	protected function get_import_data( $selected_import_index = 0 ) {
		...
		if ( ! empty( $import_files['content'] ) ) {
			$content['content'] = array(
				'title'            => esc_html__( 'Content', 'eva' ),
				'description'      => esc_html__( 'Demo content data.', 'eva' ),
				'pending'          => esc_html__( 'Pending', 'eva' ),
				'installing'       => esc_html__( 'Installing', 'eva' ),
				'success'          => esc_html__( 'Success', 'eva' ),
		...
		if ( ! empty( $import_files['widgets'] ) ) {
			$content['widgets'] = array(
				'title'            => esc_html__( 'Widgets', 'eva' ),
				'description'      => esc_html__( 'Sample widgets data.', 'eva' ),
				'pending'          => esc_html__( 'Pending', 'eva' ),
				'installing'       => esc_html__( 'Installing', 'eva' ),
				'success'          => esc_html__( 'Success', 'eva' ),
		...
		if ( ! empty( $import_files['sliders'] ) ) {
			$content['sliders'] = array(
				'title'            => esc_html__( 'Revolution Slider', 'eva' ),
				'description'      => esc_html__( 'Sample Revolution sliders data.', 'eva' ),
				'pending'          => esc_html__( 'Pending', 'eva' ),
				'installing'       => esc_html__( 'Installing', 'eva' ),
				'success'          => esc_html__( 'Success', 'eva' ),
		...
		if ( ! empty( $import_files['options'] ) ) {
			$content['options'] = array(
				'title'            => esc_html__( 'Options', 'eva' ),
				'description'      => esc_html__( 'Sample theme options data.', 'eva' ),
				'pending'          => esc_html__( 'Pending', 'eva' ),
				'installing'       => esc_html__( 'Installing', 'eva' ),
				'success'          => esc_html__( 'Success', 'eva' ),
		...
		if ( ! empty( $import_files['redux'] ) ) {
			$content['redux'] = array(
				'title'            => esc_html__( 'Redux Options', 'eva' ),
				'description'      => esc_html__( 'Redux framework options.', 'eva' ),
				'pending'          => esc_html__( 'Pending', 'eva' ),
				'installing'       => esc_html__( 'Installing', 'eva' ),
				'success'          => esc_html__( 'Success', 'eva' ),
		...
		if ( false !== has_action( 'merlin_after_all_import' ) ) {
			$content['after_import'] = array(
				'title'            => esc_html__( 'After import setup', 'eva' ),
				'description'      => esc_html__( 'After import setup.', 'eva' ),
				'pending'          => esc_html__( 'Pending', 'eva' ),
				'installing'       => esc_html__( 'Installing', 'eva' ),
				'success'          => esc_html__( 'Success', 'eva' ),
	...
	public function import_revolution_sliders( $file ) {
		...
		$this->logger->info( __( 'The revolution slider import was executed', 'eva' ) );
	...
	public function pt_importer_new_ajax_request_response_data( $data ) {
		...
		$data['message']  = esc_html__( 'Installing', 'eva' );
		$data['proceed']  = 'true';
	...
	public function after_content_import_setup() {
		...
		$shop_page       = get_page_by_title( 'Our Shop' );
		...
		if ( class_exists( 'WPCF7' ) ) {
			update_post_meta( $contact_page_id->ID, 'contact_form', '[contact-form-7 id="689" title="Contact form 1"]' );
		}
    ...
    function set_woocommerce_pages( $settings ) {
      ...
      'woocommerce_pages' => 
        array (
          'woocommerce_shop_page_id' => 'Our Shop',
		...
		eva_update_listing_expiring_dates();
	...
	public function validate_import_file_info( $import_files ) {
		...
		foreach ( $import_files as $import_file ) {
			if ( ! empty( $import_file['import_file_name'] ) ) {
				...
				$this->logger->warning( __( 'This predefined demo import does not have the name parameter: import_file_name', 'eva' ), $import_file );
			}
		}
