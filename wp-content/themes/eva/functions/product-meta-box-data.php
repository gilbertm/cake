...
/**
 * Eva_Meta_Box_Product_Data class.
 */
class Eva_Meta_Box_Product_Data {
	...
	public function product_meta_tab( $product_data_tabs ) {
		...
		$product_data_tabs['eva_attributes_extra'] = array(
			'label'  => esc_html__( 'Product Attribute', 'eva' ),
	...
	public function create_product_extra_fields( $post_id ) {
		...
		if ( ! $attributes ) : ?>
			<div id="message" class="inline notice woocommerce-message">
				<p><?php esc_html_e( 'You need to add attributes on the Attributes tab.', 'eva' ); ?></p>
			</div>

		<?php else :
			$options         = array();
			$options['']     = esc_html__( 'Default', 'eva' );
			$options['none'] = esc_html__( 'None', 'eva' );
			...
			woocommerce_wp_select(
				array(
					'id'       => 'attributes_extra',
					'label'    => esc_html__( 'Product Attribute', 'eva' ),
					'desc_tip' => esc_html__( 'Show product attribute for each item listed under the item name.', 'eva' ),
					'options'  => $options
				)
			);
