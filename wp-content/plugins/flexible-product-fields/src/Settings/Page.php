...
class Page implements Hookable, HookablePluginDependant {
	...
	private function load_translations(): array {
		return [
			...
			/* translators: %1$s: break line, %2$s: anchor opening tag, %3$s: anchor closing tag */
				...
				'<br>',
				'<a href="' . esc_url( apply_filters( 'flexible_product_fields/short_url', '#', 'fcf-settings-field-type-upgrade' ) ) . '" target="_blank" class="fpfArrowLink">',
				'</a>'
			),
			...
			/* translators: %1$s: break-line tag, %2$s: anchor opening tag, %3$s: anchor closing tag */
				__( 'Add fields using the settings at the bottom. Then it will be possible to edit the fields. %1$s%2$sRead the documentation%3$s', 'flexible-product-fields' ),
				'<br>',
				'<a href="' . esc_url( apply_filters( 'flexible_product_fields/short_url', '#', 'fpf-settings-no-fields' ) ) . '" target="_blank" class="fpfArrowLink">',
				'</a>'
			),
	...
	public function load_assets_for_settings_page() {
		...
		if ( ! $current_screen
			|| ! in_array( $current_screen->id, [ self::ADMIN_SCREEN_GROUPS, self::ADMIN_SCREEN_GROUP_EDIT ] ) ) {
			return;
		}
